...
document.getElementById('lista-coches').innerHTML = cochesOrdenados.map(hazLI).join('');
/*

// Media (o promedio) de los años de venta de los coches
*/

const funcionReductoraSuma = (acumulador,coche)=>{
  //console.log(acumulador);
  return acumulador+coche['año'];
}

const mediaAniosVenta = coches.reduce(funcionReductoraSuma,0)/coches.length;

console.log(mediaAniosVenta);



const mediaVentaCoches = coches.reduce((ac,coche, posicion)=>{
...
console.log ('del tirón: ' + mediaVentaCoches);
...
const itemsDePablito = pedidos
                            ...
                            .flat()
                            .filter(sinRepetir);
console.log('Productos de Pablito:');
...
const numeroProductosPorCategoria = productos
                  .map(p=>p.categoria)
                  .reduce((ac,c)=>{
                    const nuevoAcumulador = {...ac};
                    nuevoAcumulador[c] = nuevoAcumulador[c] ? nuevoAcumulador[c] + 1: 1;
